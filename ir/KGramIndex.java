...
public class KGramIndex {
    ...
    public void insert( String token ) {
        ...
        // YOUR CODE HERE
        //
        // checks if the token is already indexed
        ...
        int kgramNum = token.length() + 2 - getK() + 1;
        KGramPostingsEntry newEntry = new KGramPostingsEntry(newid);
        ...
        for (int i = 0; i < kgramNum; i++) {
            ...
            // System.out.println("kgrams: "+kgrams);
            if (!index.containsKey(kgrams)) {
                index.put(kgrams, new ArrayList<>());
            }
            // deduplication
            if (!index.get(kgrams).contains(newEntry)) {
                index.get(kgrams).add(newEntry);
            }
